...
from constants import abhisar_sound_path, srinivas_sound_path, aditya_sound_path, arsh_sound_path
import subprocess
import os
...
class PlayAudio:
    ...
    def play_arsh_file(cls):
        ...
        play_audio_successful = False
        try:
            speech_file_path = os.path.join(os.path.dirname(__file__), arsh_sound_path)
            print("Trying to open {}.".format(speech_file_path))
            return_code = subprocess.call(["aplay", speech_file_path])
            play_audio_successful = True
        except KeyboardInterrupt:
